...
class DataGenerator(object):
    ...
    def __init__(self,batch_size,n_labels,train_size,n_slices):
        ...
        self.steps_per_slice = self.slice_size//batch_size//2
        self.n_slices = n_slices

        self.slice_index = 0
        self.step_in_slice = 0
    ...
    def generate_data_with_label_sequence(self, dataset_images,dataset_labels, label_sequence, dataset_info):
        ...
        :return: A generator
        '''
        yield self.get_data_batch_for_labels(dataset_images, dataset_labels, label_sequence, dataset_info)
    ...
    def get_data_batch_for_labels(self, dataset_images, dataset_labels, label_sequence, dataset_info):
        ...
        part_augment_func = partial(self.get_augmented_sample_for, resize_to, dataset_name)
        image_slice = dataset_images[slice_index*self.slice_size:(slice_index+1)*self.slice_size,:,:,:]
        label_slice = dataset_labels[slice_index*self.slice_size:(slice_index+1)*self.slice_size,0]
        ...
        if step_in_slice == 0:
            slice_index = (slice_index+1)%self.n_slices

        return train_images,train_ohe_labels

    def get_augmented_sample_for(self, image, resize_to, dataset_type):
